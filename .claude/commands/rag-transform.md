...
## Step 0: Read Transform Directives (Personalization Pre-Pass)

**Before reading the retrieved context**, read `/Users/gabrielreyes/agentic-neuro/data/Sessions/transform_directives.json` using the Read tool. This file is pre-computed by Python (via `lance_retriever.py prepare_directives`) and contains all learner-aware routing decisions already resolved. If the file does not exist, proceed with generic synthesis.

**Directives fields and how to use them:**

| Field | How to Use |
|---|---|
| `has_learner_context` | If `false`, skip all personalization — proceed with generic synthesis |
...
| `skip_foundations` | If `true`, omit the surface Anchor layer, begin at mechanisms |
| `skip_anchor` | If `true` (suggested_depth ≥ 3), skip Anchor and go directly to Build mechanisms |
...
| `concepts_unknown[]` | Per-concept gap details with `concept`, `error_type`, and `misconception` fields pre-extracted. Design the Gym scenario to surface these specific failure modes |
| `cross_contamination_prone` | Boolean — if `true`, triggers the Predictive Disambiguation Pass below |
| `cognitive_pattern_alerts[]` | Process-level thinking errors (pre-filtered). If non-empty, the Gym MUST be designed to trigger this exact thinking error. Append: `<!-- COGNITIVE_PATTERN_PROBE: error_type="...", intervention_hint="..." -->` |
| `top_remediation` | If non-null, shape the Gym question to match `recommended_mode`: `drill` → test exact numbers; `socratic` → force mechanism reasoning; `disambiguation` → embed a distractor; `scenario` → clinical scenario; `scaffold` → multi-step causal question |
| `transfer_candidates[]` | Concepts with `concept`, `original_topic`, `domain` pre-extracted. If any relate to the current query's domain, design the Gym to test one in a novel clinical context. Append: `<!-- TRANSFER_TEST: concept="...", original_topic="...", new_context="..." -->` |
| `calibration_domain_alerts[]` | If non-empty and includes the current topic's domain, add a forced confidence-estimation step in the Gym |
| `confusable_pairs[]` | Pre-matched confusion pairs with `concept_a`, `concept_b`, `disambiguation_axis`, and `relevance` ("direct" or "adjacent"). If any have `relevance: "direct"`, include a **Discrimination Alert** in Critical Highlights and embed the confusable pair as a Gym trap |
| `disambiguation_required` | Boolean — if `true`, both `cross_contamination_prone` and matching confusable pairs exist → fire Predictive Disambiguation Pass |
...
2. If `concepts_unknown` has entries with `misconception` populated → design the Gym so a learner holding that exact misconception would give an identifiably wrong answer — makes the gap detectable. Name the misconception explicitly in the scenario design
3. If `skip_foundations` is `true` → omit the surface Anchor layer, begin at mechanisms
4. If `has_learner_context` is `false` → proceed with generic synthesis. Even without learner data, anticipate the most common PGY-1 errors for the topic and design the Gym to surface them
5. If `top_remediation` is non-null → the Gym question MUST target its `concept` using the `recommended_mode` framing. This takes priority over generic Gym generation but NOT over same_topic_review_due (spaced verification takes priority)
6. If `transfer_candidates` is non-empty and one candidate's domain is adjacent to the current query topic → design the Gym as a TRANSFER scenario testing that concept in the new context. Append `<!-- TRANSFER_TEST: concept="...", original_topic="...", new_context="..." -->`
7. If `cognitive_pattern_alerts` is non-empty → design the Gym to create conditions where the detected thinking error would naturally occur. Append `<!-- COGNITIVE_PATTERN_PROBE: error_type="...", intervention_hint="..." -->`
8. If `confusable_pairs` has an entry with `relevance: "direct"` → include both members in the Gym scenario. The learner must identify the discriminating feature to answer correctly.
9. If `calibration_domain_alerts` includes the current topic's domain → add a forced confidence step in the Gym: "Before answering: rate your confidence (low/medium/high) and state what would change your mind."
...
## Predictive Disambiguation Pass (fires before template rendering when triggered)

**Trigger condition:** `disambiguation_required` is `true` in `transform_directives.json`. The Python pre-processor has already matched the query against the confusion matrix — no file reading needed.

**When triggered:**
1. Read the `confusable_pairs[]` array from `transform_directives.json` (already loaded in Step 0)
2. For each pair with `relevance: "direct"`, build a disambiguation block
...
4. Fill the table **exclusively from retrieved context passages**. If a cell cannot be filled from retrieved evidence, write "not in retrieved context" — never fill from training knowledge
5. The `disambiguation_axis` from the directives provides the pre-computed separating axis. If empty, determine the most clinically useful separating axis from the retrieved context
...
1. **Inline citations required**: `[P1] [BookName, p. 42]` for textbooks (always include the passage ID tag), `[Frontier: Author et al., Year]` for web sources. Extract these from the source knowledge block. Each passage in the context is tagged with a unique ID (P1, P2, ...) — include this ID in every citation so the main agent can verify grounding.
2. **Never invent evidence.** If the retrieved context doesn't cover something, say so explicitly. Do not hallucinate sources, page numbers, or findings. **Never fill gaps from training knowledge** — signal the gap in `retrieval_gap.json` instead.
...
1. **Anticipate failure modes** — structure explanations so the wrong mental model becomes obviously wrong as you read. Preempt PGY-1 cognitive errors by design, not by appending pitfall lists.
2. **Bridge from existing knowledge** — Anchor connects to what the learner knows, then breaks/extends that schema. Never start from scratch.
3. **Pedagogical sequence: foundation → application → integration** — anatomy/pathophys first, clinical application second, advanced decision-making last.
4. **Decision frameworks over fact lists** — every Build section produces a reconstructable heuristic executable at 2 AM from memory.
5. **Rank importance** — use "If you remember nothing else:" to triage. Not all points are equal.
6. **Gym = decision tension** — force dilemmas where two correct-sounding actions conflict. Knowledge-recall questions are not Gym-worthy.
7. **Name the misconception** — state common errors directly: "The most common mistake is [X]. Here's why that fails:"
