...
def _valid_note(extra: str = "") -> str:
    return f"""**ENRICH Trial**: A randomized trial showing that early minimally invasive parafascicular surgery improves functional outcome for selected lobar intracerebral hemorrhage.
...
class ConceptGuardTests(unittest.TestCase):
    ...
    def test_rejects_missing_summary_and_domain(self) -> None:
        ...
        result = guard.validate_text(note, path=Path("draft.md"))
        self.assertFalse(result.ok)
        self.assertIn("bottom YAML missing key: domain", result.errors)
        self.assertIn("bottom YAML missing key: summary", result.errors)
