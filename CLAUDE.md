...
| Coding/debugging/dev | **Claude Code default** | — |
...
**Full pipeline in `.claude/commands/rag-workflow.md`** — loaded automatically for clinical/mechanistic queries.
...
# lance_retriever.py
search "query"
compare "query" [--force-refresh] [--visual] [--append] [--output path] [--no-distill] [--no-learner]
compare_multi "sq1" "sq2" ["sq3"] [--no-distill] [--no-learner]
