...
**Status**: Active | **Arch**: Gemini CLI + LanceDB RAG + MCP (Gmail, GCal, Chrome) + Commands
**Multi-Agent**: Python backend is agent-agnostic. Claude Code support via `CLAUDE.md` + `.claude/commands/`. Both agents share the same LanceDB, knowledge graph, and Anki infrastructure.
...
### Background Steps Policy

Steps marked **"Silent"**, **"Background"**, or **"do not narrate"** throughout these instructions are **MANDATORY steps**, not optional ones. "Silent" means: execute without mentioning it in your response to the user. It does NOT mean skip.

**Why this matters:** Every silent step writes to the knowledge graph database, a heartbeat checkpoint file, or the Obsidian vault. Missing even one silently corrupts future session state — the next session will have wrong gap data, missing Anki dedup history, or no spaced review context. These steps are load-bearing.

**Failure mode to avoid:** Skipping silent steps because they don't contribute to the visible user response. The user depends on them without seeing them.

**Execution rule:** When you see "Silent" or "do not narrate this step" before a bash command block, run the bash command, verify success, then proceed without mentioning it.

### Command Invocation Model

- Commands are invoked via `/command-name [args]` — the `.toml` file receives `{{args}}` from user text after the command name
- The `.toml` sets context; the `.md` file contains the full workflow
- Every learning command is a **multi-step interactive session** — the session does not end until the post-session hooks complete
- Sub-task delegation ("delegate to a sub-task", "use the generalist tool") spawns an isolated context that reads files from disk and writes structured output back to disk. The main agent then reads the output file.

### Tool Call Sequencing (Critical — Do Not Skip)

**Never fire two tool calls simultaneously.** Gemini 2.x crashes with "part count mismatch" on parallel function calls. Always sequential:

```
# Correct
Run preflight.sh (wait for completion).
Run lance_retriever.py (wait for completion).
Present results.

# Wrong — causes crash or hang
Run preflight.sh and lance_retriever.py at the same time.
```

This also applies to sub-task delegation: invoke one sub-task, wait for its output file, then proceed. Do not delegate multiple sub-tasks simultaneously.

**Shell commands chained with `&&` in a single bash call are fine** — they are one tool call. The restriction is against multiple simultaneous Tool.bash() invocations.

**If a shell command takes >2 minutes**: report the timeout and fall back. Don't wait indefinitely. For RAG retrieval (lance_retriever.py) and model loading, first-call latency of 30-45 seconds is expected and normal — wait for completion.

### Conditional Logic — Explicit Skip Targets

Gemini requires explicit conditional grammar. When instructions say "if X, go to Y", always use the explicit form:

**Do this:**
```
STEP 3: Check review_queue.
  - If empty -> SKIP TO STEP 5 (Targeted Remediation is unavailable)
  - If non-empty -> CONTINUE TO STEP 4
```

**Not this:**
```
If the review queue is empty, skip the Recall Bridge component.
```

Any prose conditional ("if this, do that") is lower-reliability. When in doubt, apply the skip-target form.

### Silent Step Boundary

When a section header says "Silent" or "Background", treat the section as producing NO user-visible output. Use this mental model:

- **Steps before "---" separator or before "## Step N: Present..."** → silent section, no output
- **Steps after a "Present" or "Deliver" marker** → produce user-visible output

If instructions contain a block like:
```
[Background] Run pre-flight.
[Background] Run retrieval.
[User-facing] Present the synthesis.
```
Execute all three in order, producing output only for the last.

### Model Selection

Your global default is `gemini-3-flash-preview`. Flash is excellent for fast answers, simple queries, and classification tasks. However, for commands with complex multi-step protocols and background hooks, Flash is more likely to skip or misorder steps than Pro models.

**Recommended model by command:**

| Command | Recommended | Why |
|---------|-------------|-----|
| `/intern-bootcamp` | gemini-3.1-pro | Multi-turn simulation + calibration + KG logging |
| `/rag-workflow` | gemini-3.1-pro | Transform sub-task + Gym interaction + full KG write |
| `/study-session` | gemini-3.1-pro | Component sequencing + multi-step hooks |
| `/generate-report` | gemini-3.1-pro | Multi-agent research + synthesis |
| `/intraoperative-guide` | gemini-3.1-pro | RAG + transform + KG signal |
| `/study-material` | gemini-3.1-pro | Phase 1 + Phase 2 interactive drill |
| `/anki-sync` | gemini-3.1-pro | Validation logic + image enrichment |
| `/knowledge-map` | Flash | Single DB query + format |
| `/list-textbooks` | Flash | Single DB query |
| `/explain-topic` | Flash or gemini-3.1-pro | Direct generation, no background hooks |
| `/inbox-workflow` | Flash (categorize sub-task) + gemini-3.1-pro (drafting) | Category is mechanical; drafting needs voice fidelity |

**To switch model before a complex command session**: `/model gemini-3.1-pro`
**To revert to Flash for lightweight queries**: `/model gemini-3-flash-preview`

Note: Model hints are NOT included in `.toml` descriptions — the `description` field is display-only and has no effect on model routing. Model selection is always the user's responsibility before invoking a command.

### Session-End Is Non-Negotiable

Every learning command has a session-end sequence. This sequence is the most important part of the command — it persists teaching intelligence for future sessions. The session is not complete until ALL of these have executed:
1. `heartbeat.sh --status "complete"` (saves narrative + calls `log_session_narrative`)
2. Obsidian vault file write (Review Sessions)
3. Concept extraction (Concepts/)
4. Universal Post-Session Hook (Dashboard update)

If a user ends the session abruptly, still execute steps 1-4 with the data available up to that point.
...
> **After editing any `.toml` file**: Run `/commands reload` in Gemini CLI to apply changes without restarting the session.
...
2. On approval: delegate to a sub-task → produce digest (scenario summary, diagnoses, teaching points, error patterns, open threads)
...
## Sub-Task Terminology

Where CLAUDE.md says "spawn a `general-purpose` subagent", Gemini CLI uses "delegate to a sub-task" via the `generalist` tool. The operational semantics are identical — an isolated context that returns structured output.
...
### Sub-Task Complexity Tiers

Different sub-tasks require different levels of reasoning. When delegating, consider the complexity tier:

| Sub-Task | Complexity | Rationale |
|---|---|---|
| RAG transform synthesis | **High** | Pedagogical synthesis — needs reasoning |
| Research agents (generate-report) | **High** | Web search + structured summarization |
| Concept chunking (study-material) | **High** | Classification + pedagogical structuring |
| Card drafting + validation (anki-sync) | **High** | Requires blind validation reasoning |
| Email drafting (inbox-workflow) | **High** | User-facing tone quality |
| Voice calibration (inbox-workflow) | **High** | Nuanced voice analysis |
| Procedure synthesis (intraoperative) | **High** | Surgical content quality |
| Weakness lecture (intern-bootcamp) | **High** | High-stakes educational content |
| Email categorization (inbox-workflow) | **Low** | Structured classification — simple |
| Claim extraction (anki-sync) | **Low** | SVO triple extraction — mechanical |
| Image enrichment (anki-sync) | **Low** | Search query generation — simple |
...
| `Concepts/` | Agent generated | Automated glossary. Atomic concepts automatically extracted from reports and sessions |
...
- **intern-bootcamp / study-session / rag-workflow** (standalone, not doc-anchored):
    1. **Real-Time Log**: For every significant response/answer from the user, use `knowledge_graph.py log_event` to immediately record the activity signal AND `log_study` for concept mastery.
    ...
    3. **Session-End Write**: Finalize the session file via `heartbeat.sh --session-mode --status "complete" --narrative-summary "..." --next-strategy "..."`, which automatically calls `log_session_narrative`. Then replace checkpoint content with the full session log format via file write.
    4. **Post-Session Hook**: Silently run the Universal Post-Session Hook (concept extraction + Dashboard update).
- **Doc-anchored sessions** (any command directed at a specific vault document):
...
- **Case Log Proactive Sync**: At the start of any learning command (rag, study, bootcamp, study-material), the agent MUST check `Case Log/` for new user files:
...
- **knowledge-map**: Silently triggered after any learning command via the Universal Post-Session Hook to update `Dashboard.md`.
...
All interactive learning commands (study-session, rag-workflow, intern-bootcamp, study-material) MUST check this protocol when logging gap-details with an identifiable error_type.

### gap_details Schema (MANDATORY for all learning commands)

Every gap logged must include all six fields. `root_cause` and `error_process` are required fields.
...
### Session-End Sequence (All Learning Commands)
...
**After EVERY learning command's session-end sequence** (Obsidian write complete, KG logging complete), the agent MUST silently run this hook:
...
Then regenerate `Dashboard.md` using the knowledge-map command's template (see knowledge-map.md § Obsidian Dashboard Write). Include a `## What Changed (Last Session)` section:

```markdown
## What Changed (Last Session)
- **Skill**: <command that just ran>
...
**Commands that trigger this hook**: study-session, study-material, rag-workflow, intern-bootcamp, generate-report, intraoperative-guide, anki-sync. Each command file explicitly references this hook at its session-end.

Do not narrate the Dashboard update to the user.

### Concept Extraction Protocol

After any command writes its primary output to the vault (Reports, Operative Guides, Study Material), the agent MUST extract 2-5 atomic concepts that are:
...
created: YYYY-MM-DD
extracted_from: "<command>: <topic or procedure>"
tags:
  - type/concept
  - domain/<domain>
  - source/agent
...
**Commands that trigger concept extraction**: generate-report (already does this), rag-workflow (already does this), intraoperative-guide (new), study-material (new), intern-bootcamp (new).

### INDEX.md Pre-Write Guard

Before any command writes its primary output, check for the relevant INDEX.md (`Reports/INDEX.md`, `Operative Guides/INDEX.md`, `Study Material/INDEX.md`, `Review Sessions/INDEX.md`). If absent, create it with the standard table header before writing the output file.

### Cross-Reference Discovery

Before writing any command output to vault, check for cross-reference candidates:
```bash
...
| Teaching notes per concept | `knowledge_graph.db` (`concept_mastery.teaching_notes`) | Auto-populated on unknown-to-known transition; persists effective approach |
...
| Session narratives + next_session_strategy + ZPD | `knowledge_graph.db` (`session_narratives`) | Forward-looking teaching intelligence; `session_success_rate` + `strategy_outcome` for ZPD loop; `topic_fingerprint` for overlap-based retrieval |
...
| Domain learning velocity + stagnation alerts | Derived from `signal_events.confidence_delta` via `learning_velocity` | Injected into `learner_context` when a domain shows no confidence growth over 5+ signals |
...
**Default: Answer directly using model knowledge.** The LLM's built-in training is the first-line response for clinical questions, quick overviews, explanations, and general conversation. Commands are **opt-in enhancements** invoked by explicit trigger phrases or slash commands — never auto-triggered on broad intent.

> **Rule of thumb**: If the user asks a question the model can answer from training data, just answer it. Only invoke a command when the user explicitly requests the deeper tooling OR uses a slash command.

### Mandatory Turn Gateway (Non-Slash Queries)

For every non-slash user message, run the deterministic router first:

```bash
cd /Users/gabrielreyes/agentic-neuro && source .venv/bin/activate && python3 src/gemini_query_gate.py "<user query>" --hydrate-context
```

Routing contract:
- `route = "direct"`: answer with model knowledge; do NOT run RAG/Transform
- `route = "rag-workflow"`: run full RAG workflow
- Tool-dependent routes (`anki-sync`, `knowledge-map`, etc.): invoke the matching command
- `route = "calendar"`: use GCal MCP tools

`rag-transform` is internal-only. Never invoke `/rag-transform` from user intent.

If `should_load_learner_context = true`, context hydration output must exist before answering so prior-session KG state is incorporated into the response.
...
| `/intern-bootcamp`, "drill me", "run a scenario", "night float sim", "bootcamp", "cross-cover sim", "pager sim" | **`/intern-bootcamp`** | Per command directives |
...
### Tier 3 — Default (no command needed)
...
- **Not found** → silently generate it by invoking `/study-material` on the source document.
...
**Full pipeline in command files** — invoked via `/rag-workflow` or explicit textbook search requests.
...
backfill_specificity                                 # backfill specificity_level + clinical_context for existing topics
...
# Utility scripts
src/gemini_query_gate.py "query" [--hydrate-context]   # deterministic route gate + optional KG precontext hydration
src/log_turn.sh --topic "..." --source "..." --signal-type "..." --topics "..." [--understood "..."] [--gaps "..."] [--gap-details 'JSON'] [--depth N] [--category "..."]  # single-call log_event + log_study
