...
Institution/service-origin exception: a `/brain-dump` card may preserve a local operational lesson when the prompt clearly labels it as service-specific or locally confirmable. It must not present a local convention as a universal evidence-based rule.
...
- Cards explicitly created by `/brain-dump`, or generated during `/study-review` of a `Brain Dumps/` note, must stay in `Neurosurgery::Brain Dumps`. This preserves provenance separation for institution- and personal-experience-based teaching. Tag them `brain-dump`; do not silently convert locally confirmable claims into settled universal rules.
