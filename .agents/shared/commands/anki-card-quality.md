...
Institution/service-origin exception: a card from an evaluated Brain Dump Socratic turn may preserve a local operational lesson when the prompt clearly labels it as service-specific or locally confirmable. It must not present a local convention as a universal evidence-based rule, and initial Brain Dump capture never creates cards.
...
- Portable cards generated from evaluated Brain Dump Socratic turns, or during `/study-review` of a `Brain Dumps/` note, must stay in `Neurosurgery::Brain Dumps`. Tag them `brain-dump`; do not silently convert locally confirmable claims into settled universal rules. Site-local service convention cards use service-learning routing instead.
