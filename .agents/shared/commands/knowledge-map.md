...
Dashboard: coverage summary, domain progress table, weakest/strongest areas, recent activity, patterns/calibration.

Topics: table with topic, domain, confidence, depth, encounters, last seen.

Gaps: group by urgency, decay, shallow coverage, and error clusters. Include due concepts. Offer to start `study-session`.
...
Rules: use real KG values, no stale carryover, no H1, no emojis, tables for data, Obsidian callouts for prose. Include overview, curriculum progress, study queue, concept gaps, recent activity, patterns/calibration, vault assets, and what changed last session.
