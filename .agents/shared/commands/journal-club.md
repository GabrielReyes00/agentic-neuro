...
Workflow state lives under `data/Sessions/journal_club_<slug>/`.
...
`data/Sessions/journal_club_<slug>/draft.md`. Install and validate through:

```bash
python3 src/journal_club_guard.py install --draft "data/Sessions/journal_club_<slug>/draft.md" --title "<Short Article Title>" --source-pdf "<article.pdf>" --overwrite --json
