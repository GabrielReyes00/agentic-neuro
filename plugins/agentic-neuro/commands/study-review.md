...
At startup, run the shared `study_memory.py startup-recall` command and read `startup_recall` plus `planning_brief` before teaching, including `artifact_alignment` for doc review. Do not substitute raw `summary` for session initialization.
