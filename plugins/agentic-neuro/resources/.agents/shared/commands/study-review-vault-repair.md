...
Read `retrieval_status` and `retrieval_freshness` first.

- `complete_fresh`: use one targeted discriminator, mental model, execution check, evidence anchor, or local clarification.
- `partial_fresh`: use only clearly relevant fields; rely on native knowledge for the rest.
- `partial_stale`: use only hits explicitly retained as fresh; do not use discarded stale paths or mismatched Lance generations.
- `failed` or `failed_stale`: do not pretend vault context exists; teach from native knowledge and formal sources when needed.

After using vault context, ask a near-transfer retest. Expand in the order
artifact core → prerequisite → confuser → consequence → one-hop transfer. Do
not turn a miss into a broad vault-note lecture unless Gabriel asks for a full
reveal.
