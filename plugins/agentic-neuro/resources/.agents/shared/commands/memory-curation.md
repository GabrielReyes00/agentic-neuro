...
- **Model-originated edges**: when native clinical knowledge surfaces a missing prerequisite or confusion the per-turn log does not yet evidence (the teaching doctrine's "Landscape Is A Skeleton, Not A Ceiling" rule), author the relationship with `origin: "model_proposed"` and a non-empty `rationale` instead of claim evidence. These persist distinctly and never overwrite an evidence-backed (`curated`) edge: a later curated update dominates strength, evidence, and origin. Keep them additive and auditable; do not inflate `strength` above 0.6 for a model-proposed edge.
