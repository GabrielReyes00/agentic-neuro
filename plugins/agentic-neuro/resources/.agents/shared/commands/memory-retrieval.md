...
Routine `study-review` does not load this audit contract at entry. It uses
`tutor-state.md` and `profile=tutor`. Load this file only for point-of-need node
drilldown, learner-model audit, identity repair, or a compact state that remains
clinically incoherent.
...
```bash
python3 src/study_memory.py startup-recall --lens service --service "<service>" --site "<site>" --session "$SESSION_TS"
```
...
Startup profiles:
- `--profile tutor`: canonical routine `study-review` surface. It emits
  `tutor_state_v1`, including an active map window, exact high-priority learner
  evidence, phase controller, one-hop context candidates, learner expectation
  profile, and pointers to node/audit drilldown. It omits the rich
  `planning_brief` from model context.
...
- **`orient_skip`**: ORIENT is bypassed deterministically when the learner already holds a schema — `reason` is `all_entry_nodes_have_prior_exposure` (every entry exposed), `predominant_prior_exposure` (exposed entries ≥ 60% and ≥ 3), or `substantial_deepenable_core` (≥ 4 exposed-superficial/open entry nodes). The few still-unexposed entries fold into DEEPEN; do not re-orient a clearly-engaged topic.
...
2. **`artifact_alignment`**: document coverage, remaining artifact-native concepts, graph-only bridge candidates, horizon-expansion candidates, and unresolved map gaps.
3. **`teaching_priorities`**: the ranked blend of open gaps, recent repairs, and stale scaffolds.
4. **`contextual_frontier`**: capped related-context candidates. Accept only candidates central to the requested document.
5. **`question_design_bias`** and **`domain_patterns`**: small shaping signals, not a mandate.
6. **`anki_overlay`**: optional scoped Anki feedback. Use it only to shape sequencing, scaffolds, and redundant-card avoidance after the document curriculum and SQLite priorities are set.
...
- A `retention_check` promotes a repaired claim only from a different session after its scheduled `next_due_ts`; an early check remains `repaired_same_session` and records `retention_not_due`.
- Response time and observed confidence may shape probe difficulty, but neither independently creates claim durability or mastery depth.
...
Guardrails:
- SQLite precedence: Anki shapes the queue; it does not own the queue. Anki never overrides `handoff`, `open_first`, `recent_repairs`, urgent `due_claims`, or artifact-native requested-document priority.
...
Canonical `study-review` writes use `assess-turn --stdin`. One raw learner
response remains one exchange even when it contains several independently
graded claims. Rich dimensions live in `claim_assessments`; established
longitudinal scoring and scheduling remain in `claim_results`/`claim_state`.
`pending_adjudication` means the tutor could not safely grade and therefore
must not create mastery or gap state. `turn_assessments.idempotency_key` makes
retries non-duplicating. See `study-review-turn.md` for the typed schema.
...
**Four-layer field discipline.** A log entry has four layers, each with one job;
keep them separate. This is the canonical statement; turn-level contracts may
summarize it but do not own it:
