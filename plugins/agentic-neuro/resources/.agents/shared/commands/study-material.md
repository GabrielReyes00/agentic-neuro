...
are unavailable, draft to `$RUN_DIR/draft.md` and install
through the guard.
...
```bash
python3 src/study_material_guard.py install --draft "$RUN_DIR/draft.md" --title "<Topic Title>" --json
