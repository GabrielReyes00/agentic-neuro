...
- `data/vault_index.lance` owns the dedicated LanceDB table `vault_notes`. It stores embedded section payloads from the vault and is physically separate from the textbook table `neurosurgery_v4`; `NEURO_VAULT_LANCE_DIR` may override the root for tests or migrations.
...
The former textbook-nested `neurosurgery_v4.lance/vault_notes.lance` migration
copy has been retired. `data/vault_index.lance` is the only valid vault-vector
store. Treat recreation of the former path as a health failure; never read or
write it.
