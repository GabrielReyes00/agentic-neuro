...
- `service_gaps` leads the queue; `conventions` are locally confirmable reminders;
  ...
  competency targets; `pending_review_candidates` are low-weight future probes,
  not mastery evidence. Honor `weighting_policy: service_primary_formal_capped`.
- Read `rotation`, `rubric_progress`, and `data_quality_warnings` before using the
  service lens. Never guess a generic-domain candidate into a service; classify
  it explicitly before it can steer rotation teaching.
