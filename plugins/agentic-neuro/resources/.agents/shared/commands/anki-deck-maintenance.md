...
This is a manifest workflow. Use the runtime-provided `RUN_DIR` for exports and
the approved edit plan. Export and assessment are read-only. Stop at the
approval node before any live note edit, deck move, suspension, or cache rebuild.
...
python3 src/anki_deck_tools.py export \
  ...
  --output "$RUN_DIR/live-export-before.json"
```
...
python3 src/anki_deck_tools.py export \
  --query "deck:Neurosurgery*" \
  --output "$RUN_DIR/live-export-after.json"
```
...
Register both exports and the approved plan as `audit` artifacts. The live deck
remains the deliverable and source of truth; the manifest stores only the
run-scoped evidence needed to verify what changed.
