...
description: Multi-agent research report generator — decomposes a neurosurgical topic into parallel web-research subagents, supplements with textbook RAG, and synthesizes a comprehensive reference report. Invoke via /generate-report or when the user explicitly requests a full research report — "generate a report on", "research report on", "comprehensive review of". For general questions about a topic, answer from model knowledge instead.
---
...
|---------|--------|----------------|
| Before Phase 1 research planning | `./src/preflight.sh "$QUERY"` — read learner_context.json | Never skip |
| Before vault write | INDEX.md pre-write guard (check Reports/INDEX.md exists) | Never skip |
| After vault write | Cross-reference discovery (ls vault folders, generate wikilinks) | Never skip |
| After vault write | Concept extraction — 2-5 concepts to `Concepts/` | Never skip |
| After concept extraction | Universal Post-Session Hook (apply_decay + dashboard) | Never skip — FINAL mandatory step |

**Execution rule:** Each hook runs without narration. Verify command success before proceeding to the next hook.
...
**Primary sources:** PubMed (historical reviews, anniversary articles), Google Search for seminal papers and biographical context
...
### Step 2.2: Launch Research Agents Sequentially

Launch each research agent as a sequential sub-task (one at a time — do NOT run agents in parallel; Gemini CLI crashes on parallel sub-task calls). Wait for each agent to write its output file before launching the next.
...
3. UpToDate — current clinical recommendations (via Google Search, often accessible in snippet form)
...
8. Peer-reviewed journal articles — via Google Search for specific claims
...
Use Google Search with targeted PubMed queries as your PRIMARY method for medical literature:
- Search with site:pubmed.ncbi.nlm.nih.gov for structured article discovery
- Use MeSH terms when possible (e.g., "anterior choroidal artery"[MeSH] aneurysm)
- For full text, search site:ncbi.nlm.nih.gov/pmc for PubMed Central open-access articles
- Fetch article pages directly to extract abstracts, author lists, and DOIs

SEARCH STRATEGY: Start each module with PubMed-targeted Google searches using MeSH terms, then supplement with broader web searches for guidelines, device specs, or sources not indexed in PubMed.
```
...
In addition to the module-based agents, ALWAYS launch a dedicated Historical Context agent. Launch it sequentially after the last module-based agent completes — do NOT attempt to launch it in parallel. This agent receives a specialized prompt:
...
[Include the same Source Priority, PubMed Search Strategy, Write Protocol, and Quality Standards blocks as other agents]
...
- Google Search: search for "[topic] history neurosurgery", "[topic] first described", "[topic] seminal paper"
- Look for dates, names, institutions, and the narrative arc of discovery.
```

After each agent completes, report progress to the user:
```
Agent [N] ([name]) complete — [M] words across [K] modules. Starting Agent [N+1]...
```

When all agents including the Historical Context agent have completed:
```
All [N] research agents complete. Beginning textbook RAG supplement.
```
...
2. For each module, perform the research directly using Google Search and web fetching
...
## Phase 3: Verify Agent Output

After each sub-task completes (sequential execution — each one finishes before the next starts):

1. Read the agent's output file
2. Verify it has substantive content (not just headers) and ends with "STATUS: COMPLETE"
3. Report completion to the user: `"Agent N (Name) complete — M words."`

**If an agent produces minimal/no content:**
1. Read whatever it produced to understand what went wrong
2. Re-delegate to a new sub-task with a refined prompt:
   - Pre-load any partial findings already written
   - Provide more specific search queries for the failing modules
   - Start with: "Write your pre-loaded findings FIRST, then search for more"
3. Report the retry to the user

**After ALL sub-tasks complete**, proceed to Phase 4.
...
Before writing the report, check for existing vault content to cross-reference:
...
Match filenames against the current report topic using keyword overlap. Store matches for the `## Related in This Vault` section appended to the report. Also use matches for inline wikilinks within the report body where key clinical terms match existing Concept notes (e.g., `the [[Concepts/Neurosurgery Consult Workflow|consult workflow]] applies here...`).

### Step 5.3: Write the Report

Write to `/Users/gabrielreyes/Documents/Obsidian/agentic-neuro/Reports/<topic_slug>.md`.
...
```bash
rm -rf /tmp/neuro-report-XXXXX && rm -f data/Sessions/learner_context.json
```
...
Run the Universal Post-Session Hook (see GEMINI.md) to update Dashboard.md. This ensures the Dashboard reflects the new report's topics and any knowledge graph changes from Step 6.3.
