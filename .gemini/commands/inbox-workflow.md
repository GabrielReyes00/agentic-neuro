...
## Phase 1: Collect & Categorize Emails (Sub-task)

Delegate to a sub-task to handle all email fetching and categorization. Raw email bodies stay in the sub-task's context and never enter the main agent.
...
After the sub-task returns the JSON, use it to render the Phase 2 dashboard (see below). Do NOT re-fetch or re-read any emails.
...
Before drafting the first response, delegate to a sub-task to perform voice calibration. This runs once per session — subsequent drafts reuse the cached result.
...
After the sub-task returns the voice profile, hold it in memory for use in all Phase 4 drafts. If the sent examples conflict with the static Voice Profile (Section above), **defer to the real examples** — they reflect current practice.

### Steps 1-3: Sequential Drafting Loop (Sub-task per Email)
...
**Step 3: Draft Response (Sub-task)** — Delegate to a sub-task to draft the reply. The sub-task reads the full email body in its own context, keeping raw email content out of the main conversation.
...
Present the sub-task's draft to the user:
...
2. **"revise: [feedback]"** -> Re-invoke the drafting sub-task with the feedback appended to Gabriel's response guidance. Present the new draft for approval. Repeat until approved or skipped.
