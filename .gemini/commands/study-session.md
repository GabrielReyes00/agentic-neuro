...
Execute the full RAG workflow (Step 0-5 from GEMINI.md) on the selected gap topic. Use `neuro-scaffold` template by default, or `board-exam` if the user asked for board review.
...
| Remediation | nimodipine dosing — correct on 2nd attempt | Partially resolved |
