...
### 4. Critical Caveats (The 'Never-Events')
...
## Trigger Logic (Three-Layer Architecture)
...
1. Run retrieval in parallel:
   ...
   ```
2. Delegate to a sub-task with the `rag-transform` instructions (read `.gemini/commands/rag-transform.md`), using `TEMPLATE=neuro-scaffold` and the procedure as the query.
