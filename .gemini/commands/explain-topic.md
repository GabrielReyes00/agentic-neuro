...
Before writing the explanation, hydrate KG context for the topic:

```bash
cd /Users/gabrielreyes/agentic-neuro && source .venv/bin/activate && python3 src/knowledge_graph.py context "<topic>" --output data/Sessions/learner_context.json
```

Use this context only to calibrate explanation depth and emphasis:
- Prior gaps or confusions on this topic get explicit discriminations.
- Previously mastered concepts get brief verification-level treatment.
- If context is missing/unavailable, continue with a normal explanation.
...
**brief**: Output The Core only, followed by one sentence stating what knowing this precisely changes in practice. Target ~150 words. No Discriminations, no Mechanism deep-dive. Use case: fast orientation when the reader already has significant surrounding context.
...
### Optional Socratic Question (deep only)
