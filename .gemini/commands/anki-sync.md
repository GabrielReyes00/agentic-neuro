...
This command converts the current session transcript into high-yield Anki flashcards. You (the agent) handle all LLM reasoning natively. Python is only invoked for local database and AnkiConnect operations.
...
## Steps 3-4: Claim Extraction & Novelty Filtering (Sub-task)

Delegate to a sub-task with the following prompt:
...
## Steps 5-6: Card Drafting & Validation (Sub-task)
...
After the sub-task returns, report the counts briefly and proceed to Step 7.
