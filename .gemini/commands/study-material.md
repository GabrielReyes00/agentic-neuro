...
|---------|--------|----------------|
| Phase 1 Step 0 (before any extraction) | `./src/preflight.sh "<inferred topic>"` — read learner_context.json | Never skip |
| Before vault write (Phase 1 Step 3) | INDEX.md pre-write guard + cross-reference discovery | Never skip |
| After EVERY Phase 2 learner response | `./src/log_turn.sh` (single call writes both tables) | Never skip |
| Every 3 user turns in Phase 2 | `heartbeat.sh --doc "..." --doc-type "..."` checkpoint | Never skip |
| After Phase 2 session ends | Final heartbeat (complete) + upsert `<slug>_review.md` | Never skip |
| After Obsidian write | Concept extraction to `Concepts/` | Never skip |
| After concept extraction | Universal Post-Session Hook | Never skip — FINAL mandatory step |
...
**Vault `.md` files** (Reports, Operative Guides) are valid input sources. For these, skip PPTX/PDF extraction (Step 1) and read the vault document directly. Treat each `##` heading as a slide/page and each bullet as a body item. Proceed from Step 2 onward identically.
...
Read `data/Sessions/learner_context.json`. Use it to:
- Identify concepts the user already knows well (don't over-test these — include 1-2 verification questions max)
- Identify known gaps or confusable pairs (generate extra discrimination questions for these)
- Note calibration alerts (if overconfident in a domain, include harder questions that probe depth)

Do not narrate this step.

### Step 1 — Exhaustive Content Extraction & Concept Chunking (SECTION-BY-SECTION LOOP)

**CRITICAL: To prevent "generation laziness" and ensure exhaustive coverage of dense clinical reports, PPTX, or PDF files, you MUST process the document in a section-by-section extraction loop. NEVER try to summarize a large file in a single pass.**
...
**Extraction Protocol (The Loop):**
1. Read the input document. Divide it into logical chunks (e.g., 2-3 pages, 5-7 slides, or 1-2 `##` heading sections of an `.md` report).
2. For *each chunk*, extract EVERY testable clinical nuance, anatomic detail, and mechanism as a distinct concept. To maintain context limit, you can delegate each chunk's extraction to a sub-task via the `generalist` tool.
3. Group these concepts into Teaching Units (TU). A highly dense report may have 10-15 Teaching Units.
4. Compile all chunked outputs into a single master Concept Map.

**For PPTX/PDF files:**
- Read the file and extract per-page/slide. Capture all text, notes, and visual descriptions.
- Group into chunks and extract exhaustively.

**For Vault `.md` files:**
- Treat each `##` heading as a chunk.
- Extract every bolded term, clinical pearl, risk factor, and anatomical relationship.

### Step 2 — Complexity Classification

For each extracted concept across all your chunked Teaching Units, classify it by complexity.
...
**Scale with content**: You MUST extract ALL testable concepts. A comprehensive 3,000-word clinical report should yield 30-50+ questions. A 40-slide deck on brain anatomy may generate 60+ questions. Let the content dictate, and use the chunking loop to guarantee no loss of fidelity.

Write the compiled output to `data/Sessions/concept_map_<topic_slug>_<YYYYMMDD>.json`:
```json
{
  "teaching_units": [
    {
      ...
      "clinical_corressions": ["AComm aneurysm is the most common location for aneurysmal SAH"]
    }
  ],
  "total_concepts": 64,
...
Write each to `Concepts/<Name>.md` per the Concept Extraction Protocol in GEMINI.md. Use `extracted_from: "study-material: <document title>"`. Each concept should wikilink back to the Study Material document.
...
If the user says "drill me on brain anatomy lab 1" or "continue where we left off on [topic]", check for an existing file in `/Users/gabrielreyes/Documents/Obsidian/agentic-neuro/Study Material/` matching the topic. If found, read the document and run the Drill Pre-Flight (doc_status + Review Session check) before starting Phase 2 — no need to regenerate.
...
3. Trigger the `/anki-sync` command with this scoped transcript

---

## Context Compression

If the drill session exceeds 12 turns, follow the standard compression protocol from GEMINI.md:
